...
def null_text_tokens(item, null_text_id):
    # null any text in a sample item for the unconditional CFG branch - flags untouched

    if isinstance(item, tuple) and is_int_tensor(item[0]):
        return (torch.full_like(item[0], null_text_id), *item[1:])

    if is_int_tensor(item):
        return torch.full_like(item, null_text_id)
...
def to_named_modality_items(
    samples: ModalitySample | list[ModalitySample]
) -> ModalitySample | list[ModalitySample]:
    # swap modality tuple items with their named equivalents, so a sample can be read directly
    # by attribute, e.g. `samples[0][2].tensor`

    if samples and isinstance(samples[0], list):
        return [to_named_modality_items(sample) for sample in samples]

    for i, sample in enumerate(samples):
        if isinstance(sample, tuple):
            named_item = to_named_modality_item(sample)

            if exists(named_item):
                samples[i] = named_item
...
def print_modality_sample(
    ...
    for sample in modality_sample:
        if isinstance(sample, ModalityItem):
            output.append((f'modality:{sample.modality_type}', sample.tensor.shape))
        elif isinstance(sample, tuple):
            modality_type, sample, *_ = sample
...
def flow_weight_vector(instance_weights: list[float | Tensor], flow_tensors: list[Tensor], channel_first: bool) -> Float['t']:
    # per instance loss weights -> a per-token weight vector, already shaped to broadcast over
    # the packed flow tiles along the feature axis

    per_token = []

    for weight, flow_tensor in zip(instance_weights, flow_tensors):
        if is_tensor(weight):
            per_token.append(weight.reshape(-1))
        else:
            channel_axis = 0 if channel_first else -1
            token_count = flow_tensor.numel() // flow_tensor.shape[channel_axis]
            per_token.append(torch.full((token_count,), weight, device = flow_tensor.device, dtype = torch.float))

    weight = cat(per_token)

    if channel_first:
        return rearrange(weight, 't -> 1 t')

    return rearrange(weight, 't -> t 1')
...
def excluded(start, end):
    ...
    def mask_fn(b, h, q_idx, kv_idx):
        return ~(((q_idx >= start) & (q_idx < end)) | ((kv_idx >= start) & (kv_idx < end)))

    return mask_fn

def transfusion_attn_mask(modalities: Int['b m 3'], excluded_spans: Int['b m 2'] | None = None):
    ...
    def mask_mod(b, h, q_idx, kv_idx):
        ...
        if exists(excluded_spans):
            for start, end in excluded_spans[b]:
                mask = mask & excluded(start, end)(b, h, q_idx, kv_idx)
...
def naive_attn_mask(
    ...
    modalities: Int['b m 3'],
    device = None,
    excluded_spans: Int['b m 2'] | None = None
) -> Bool['b i j']:
    ...
    mask = is_causal | is_modality.any(dim = 1)

    if exists(excluded_spans):
        starts, ends = excluded_spans.unbind(dim = -1)
        is_excluded = (
            einx.greater_equal('i, b m -> b m i', seq, starts) &
            einx.less('j, b m -> b m j', seq, ends)
        )
        is_excluded = is_excluded.any(dim = 1)
        mask = mask & ~einx.logical_or('b i, b j -> b i j', is_excluded, is_excluded)
...
def apply_fn_modality_type(
    ...
    # standardize tuples to (<modality_type>, <modality_tensor>[, <loss weight>]);

    def to_standardized(m):
        if not isinstance(m, tuple) or not is_tensor(m[0]) or not m[0].is_floating_point():
            return (0, m) if (is_tensor(m) and m.is_floating_point()) else m

        # bare (tensor[, loss weight]) form - implicitly modality type 0

        return (0, m[0], m[1] if len(m) > 1 else None)

    modalities = [to_standardized(m) for m in modalities]

    # filter by type - text (tensor[, flag]) tuples have an int tensor first, so never match

    modalities, inverse_filter = filter_with_inverse(
        lambda el: isinstance(el, tuple) and isinstance(el[0], int) and el[0] == modality_type,
        modalities
    )

    # keep the type and spec separate so both survive the transform

    type_and_specs = [(m[0], m[2] if len(m) > 2 else None) for m in modalities]
    modalities = [m[1] for m in modalities]
    ...
    if return_untransformed:
        out = [
            (modality_type, transformed_m, *((spec,) if exists(spec) else ()), prev_m)
            for (_, spec), transformed_m, prev_m in zip(type_and_specs, out, modalities)
        ]
    else:
        out = [
            (modality_type, transformed_m) if not exists(spec) else (modality_type, transformed_m, spec)
            for (_, spec), transformed_m in zip(type_and_specs, out)
        ]
...
def sample_text_token(logits, temperature = 1.0, min_p = 0.1):
    if temperature == 0.:
...
def parse_forced_modality_at_start(force_modality_at_start):
    """ normalize `force_modality_at_start` to a (modality_type, shape) tuple """

    if isinstance(force_modality_at_start, tuple):
        return force_modality_at_start

    return force_modality_at_start, None
    ...
    def forward(
        ...
        if needs_masking:
            ...
            if exists(modality_positions):
                ...
                if should_use_flex_attn:
                    transfusion_mask_fn = transfusion_attn_mask(modality_positions, excluded_spans = excluded_spans)
                    block_mask = create_block_mask(transfusion_mask_fn, B = batch, H = None, Q_LEN = seq_len, KV_LEN = seq_len, _compile = True, device = device)
                    attn_mask_kwargs.update(block_mask = block_mask)
                else:
                    attn_mask = naive_attn_mask(seq_len, modality_positions, excluded_spans = excluded_spans, device = device)
        ...
        if not exists(is_any_modality) and exists(modality_positions):
            num_modalities = modality_positions[..., 0].amax().item() + 1
            is_any_modality = modality_positions_to_is_modality_mask(seq_len, modality_positions, num_modalities = num_modalities).any(dim = 1)
...
class Transfusion(Module):
    ...
    def get_modality_shape_from_seq(
        ...
        forced_modality_shape: tuple[int, ...] | None = None
    ) -> tuple[int, ...]:
        """ determine the shape of the modality to be decoded, either from the forced shape, the meta string tokens after the [som] token, or fall back to the default shape """

        modality_shape = default(forced_modality_shape, fixed_modality_shape)
        ...
        if maybe_meta_tensor.numel() > 0 and not exists(forced_modality_shape):
    ...
    def maybe_force_modality_at_start(
        self,
        sample: ModalitySample,
        force_modality_at_start: int | tuple[int, tuple[int, ...]] | None
    ) -> tuple[ModalitySample, int | None, tuple[int, ...] | None]:
        """ append the [som] (and optional [meta] + shape) tokens to the end of the prompt so that sampling starts off by decoding the forced modality, returning the forced modality type and shape """

        forced_modality_id, forced_modality_shape = parse_forced_modality_at_start(force_modality_at_start)

        if not exists(forced_modality_id):
            return sample, forced_modality_id, forced_modality_shape

        device = self.device
        mod = self.get_modality_info(forced_modality_id)

        if exists(forced_modality_shape):
            modality_shape_str = join([*map(str, forced_modality_shape)], ',')
            meta_tensor, shape_tokens, som_tensor, _ = get_cached_meta_tokens(self, device, modality_shape_str, forced_modality_id)

            forced_tokens = cat((meta_tensor, shape_tokens, som_tensor))
        else:
            forced_tokens = tensor([mod.som_id], device = device)

        last_part = sample[-1]

        if is_int_tensor(last_part):
            sample[-1] = cat((last_part, forced_tokens))
        else:
            sample.append(forced_tokens)

        return sample, forced_modality_id, forced_modality_shape

    def prepare_prompt_sample(
        self,
        prompt: ModalitySample | Tensor | tuple[int, Float['...']] | None,
        force_modality_at_start: int | tuple[int, tuple[int, ...]] | None
    ) -> tuple[ModalitySample, int | None, tuple[int, ...] | None]:
        """ normalize a prompt into a full sample of [sos] + prompt parts, with any forced modality at start, returning the forced modality type and shape """
        ...
        if is_int_tensor(prompt): # is text only prompt
            ...
            modality_type, modality, *_ = prompt
        ...
        if isinstance(prompt, list):
            ...
            # a raw modality ending the prompt is closed with its [eom], so that text decoding
            # resumes from the end of the modality as it would in training

            if prompt and not is_tensor(prompt[-1]):
                last_modality_type, _, *_ = prompt[-1]
                eom_id = self.get_modality_info(last_modality_type).eom_id
                prompt.append(tensor([eom_id], device = device))

        # sos, plus take care of prompt being zero dimensions and moving to device
        ...
        # maybe force the first generated thing to be a modality of a specific type, and shape if given

        return self.maybe_force_modality_at_start(
            sample,
            force_modality_at_start
        )

    def get_modality_transition(
        self,
        seq: Int['n'],
        fixed_modality_shape: tuple[int, ...] | None,
        forced_modality_id: int | None,
        forced_modality_shape: tuple[int, ...] | None
    ) -> tuple[int, tuple[int, ...]] | None:
        """ if the last token of `seq` is a [som] token, return the (modality type, shape) of the modality to decode next, otherwise None """
        ...
        modality_shape = self.get_modality_shape_from_seq(
            seq,
            curr_modality_id,
            fixed_modality_shape,
            forced_modality_shape if curr_modality_id == forced_modality_id else None
        )

        return curr_modality_id, modality_shape
    ...
    def decode_modalities(
        ...
        """ post process the sampled modalities, running them through the modality decoders if they exist """
    ...
    def sample_one(
        ...
        cache_kv = False,
        fixed_modality_shape: tuple[int, ...] | None = None,
        force_modality_at_start: int | tuple[int, tuple[int, ...]] | None = None,
        ...
        if self.num_text_tokens == 0:
            ...
            return self.generate_modality_only(batch_size = 1, modality_type = forced_modality_id, fixed_modality_shape = forced_modality_shape)

        # take care of prompt being a raw tensor, either text or raw modality (image, video, actions, latents, etc)

        modality_sample, forced_modality_id, forced_modality_shape = self.prepare_prompt_sample(
            prompt,
            force_modality_at_start
        )
        ...
        *_, last_modality_sample = modality_sample
        ...
        def maybe_transition_to_modality_decoding(seq):
            ...
            transition = self.get_modality_transition(seq, fixed_modality_shape, forced_modality_id, forced_modality_shape)

            if not exists(transition):
                return

            curr_modality_id, modality_shape = transition
            is_decoding_text = False

        # determine if to transition from start, if the prompt ends with a [som]

        if is_tensor(last_modality_sample):
            ...
            while curr_length <= max_length:
                ...
                if is_decoding_text:
                    ...
                    *_, seq = modality_sample
                    ...
                    sampled = sample_text_token(logits, text_temperature, text_min_p)
                    ...
                    if use_cfg:
                        ...
                        uncond_history = [
                            null_text_tokens(item, self.null_text_id)
        ...
        return self.decode_modalities(modality_sample)
    ...
    def sample_many(
        ...
        # the first token of each sample, the same forward `sample_one` uses for its first text
        # decoding step. `return_embed` is set so the processing does not re-add the [meta] [shape]
        # [som] [eom] tokens around prompted modalities, and the prompted modalities are
        # conditioned at a time of 1., as `sample_one` conditions previously decoded ones

        max_past_modalities = max((state.num_past_modalities for state in states), default = 0)

        init_times = None

        if max_past_modalities > 0:
            init_times = torch.ones((len(states), max_past_modalities), device = device)

        (embed, _), (init_cache_tensor, _) = self.forward(
        ...
        def maybe_transition_to_modality(state):
            transition = self.get_modality_transition(state.curr_seq, fixed_modality_shape, forced_modality_id, forced_modality_shape)
        ...
        for state in states:
            maybe_transition_to_modality(state)
        ...
        for ind, (state, seq_len) in enumerate(zip(states, sample_seq_lens)):
            ...
            if state.phase == 'text':
                sampled = sample_text_token(logits[ind, seq_len - 1], text_temperature, text_min_p)
        ...
        def step_text(group):
            ...
            for ind, (seq_len, state) in enumerate(zip(cache_seq_lens, group)):
                ...
                if maybe_transition_to_modality(state):
        ...
        return self.decode_modalities(samples)
    ...
    def forward(
        ...
        if self.training and prob_uncond > 0 and isinstance(modalities, list):
            ...
            modalities = [
                [null_text_tokens(item, self.null_text_id) for item in sample] if is_uncond else sample
                for sample, is_uncond in zip(modalities, rand_mask)
            ]
        ...
        for batch_modalities in modalities:
            ...
            for ind, modality in enumerate(batch_modalities):
                ...
                if parsed.kind != 'modality':
                    continue

                # standardize bare float tensors to a type 0 modality tuple

                if not isinstance(modality, tuple):
                    modality = (parsed.modality_type, parsed.tensor)
        ...
        if return_loss:
            ...
            if exists(loss_weights):
                # align the weights with the labels: labels[i] is the token at buffer position i + 1

                text_weights = loss_weights[:, 1:]
            else:
                text_weights = None
        ...
        elif decoding_text_or_modality == 'modality':
            # first modality decode with no cache - the transformer's own fallback mask is
            # hardcoded to `num_modalities = 1` and would be all zeros for any modality type
            # beyond 0

            is_any_modality_when_decoding = is_any_modality
        ...
        # attention
        # during decoding all tokens are of one kind (text or modality); when training, the
        # per-token signal is derived from the modality positions

        is_any_modality = default(is_any_modality_when_decoding, is_any_modality)
        ...
        text_loss = F.nll_loss(
            rearrange(text_logits, 'b n l -> b l n').log_softmax(dim = 1),
            text_labels,
            ignore_index = self.ignore_index,
            reduction = 'none'
        )

        if exists(text_weights):
            # continuous per-token loss weights - 0 positions contribute nothing

            text_loss = (text_loss * text_weights).sum()
        else:
            text_loss = text_loss.sum()

        text_loss = text_loss / total_tokens

        # calculate flow losses

        flow_losses = []

        modality_weight_vectors = {}
        ...
        for modality_id, (pred_flow, is_one_modality) in enumerate(zip(pred_flows, is_modalities.unbind(dim = 1))):
            ...
            modality_weight_vectors[modality_id] = flow_weight_vector(flow_weights[modality_id], flows[modality_id], mod.channel_first_latent)

            mse = (modality_pred_flow - modality_flows) ** 2

            flow_loss = (mse * modality_weight_vectors[modality_id]).sum() / total_tokens
        ...
        # if the batch contains no modalities, there is nothing to compute a flow loss on

        flow_loss = self.zero if not flow_losses else sum(flow_losses)
        ...
        if need_velocity_matching:
            ...
            for modality_id, (ema_pred_flow, pred_flow) in enumerate(zip(ema_pred_flows, pred_flows)):
                ...
                mse = (pred_flow - ema_pred_flow) ** 2

                velocity_match_loss = (mse * modality_weight_vectors[modality_id]).sum() / total_tokens

                velocity_match_losses.append(velocity_match_loss)

            if velocity_match_losses:
                total_loss = (
                    total_loss +
                    sum(velocity_match_losses) * self.velocity_consistency_loss_weight
                )
